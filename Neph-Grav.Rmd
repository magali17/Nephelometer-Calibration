...
agg1 <- PM[c("StationCode", "SamplesGrav",  "SamplesLS")] #"SamplesTSP",
agg.max <- aggregate(data=agg1, .~StationCode, FUN=max)

PM <- PM[!colnames(PM) %in% c("SamplesGrav",  "SamplesLS")] #"SamplesTSP",
...
```{r}
pdf("/Users/magaliblanco/Everything/School/PhD_UW/Courses/1. Aut 2017/ENVH 595 Rotation A/data-cleanup/output/2009 BW & CE.pdf", width = 8, height=10.5)

df <- Grav.LS[Grav.LS$Year.x=="2009" & (Grav.LS$StationCode=="BW" | Grav.LS$StationCode=="CE"), c("ObservedDate", "StationCode", "Location", "Grav.Avg", "LS.Avg", "QmuCode.x", "QmuCode.y")]

stat1 <- c("BW", "CE")
for (i in 1:length(stat1)){
mult.fact <- 1
p1 <- ggplot(data = df[df$StationCode==stat1[i],], aes(x=ObservedDate, y=Grav.Avg)) + geom_line(alpha=.4, aes(colour=as.factor(QmuCode.x))) + geom_line(aes(y = LS.Avg*mult.fact, colour="bscat")) + scale_y_continuous(sec.axis = sec_axis(~./mult.fact, name = "bscat"))+ geom_smooth(se=F) + geom_smooth(se=F, aes(x=ObservedDate, y=LS.Avg)) + theme(legend.position = "bottom") + labs(x="Date", y= "Gravimetric  ug/m3", subtitle= paste("station ", stat1[i], sep="")) 

p2 <- ggplot(data = df[df$StationCode==stat1[i],], aes(y=LS.Avg, x=Grav.Avg, colour=as.factor(QmuCode.x))) + geom_point() + theme(legend.position = "bottom") + labs(y="bscat", x= "Gravimetric  (ug/m3)", subtitle= paste("station ", stat1[i], sep="")) + geom_abline(aes(colour="One-to-One", intercept = 0, slope = 1)) + stat_smooth(aes(colour="Best Fit"), method="lm", se=F)
...
Compare Partisol & Dichot methods (QMU: 50-51, 36) to other gravimetric methods when collocated BY STATION
-only looking at stations where grav & LS are also colocated 
? look at all data? (use PM instead of Grav.LS)
```{r table 4c ref vs other grav methods by station}
ref.QMUs <- c(36, 50, 51)
...
```{r table 4d ref vs others}
uniq.QMU.x <- unique(Grav.LS[c("QmuCode.x", "Method.x", "Quantity.x")])

table4d <- data.frame(QMU.x=uniq.QMU.x$QmuCode.x,
                     Method.x = uniq.QMU.x$Method.x,
                     Quantity.x = uniq.QMU.x$Quantity.x,
                     Pairs=rep(NA, length(uniq.QMU.x$QmuCode.x)),
                     slope=rep(NA, length(uniq.QMU.x$QmuCode.x)),
                     int= rep(NA, length(uniq.QMU.x$QmuCode.x)),
                     R2=rep(NA, length(uniq.QMU.x$QmuCode.x)))
#overall correlation
lm1 <- summary(lm(data=Grav.LS, Grav.Avg~LS.Avg))

for (i in 1:length(uniq.QMU.x$QmuCode.x)){
  df <- Grav.LS[Grav.LS$QmuCode.x==uniq.QMU.x$QmuCode.x[i],]
  lm1 <- summary(lm(data=df, Grav.Avg~LS.Avg))
  
  table4d$Pairs[i] <- length(df$ObservedDate)
  table4d$slope[i] <- round(lm1$coefficients[2,1],2)
  table4d$int[i] <- round(lm1$coefficients[1,1],2)
  table4d$R2[i] <- round(lm1$r.squared,3)
}
...
```{r table 4e}
uniq.QMU <- unique(Grav.LS[c("QmuCode.x", "Method.x", "Quantity.x", "QmuCode.y", "Method.y", "Quantity.y")])

table4e <- data.frame(QMU.x=uniq.QMU$QmuCode.x,
                     Method.x = uniq.QMU$Method.x,
                     Quantity.x = uniq.QMU$Quantity.x,
                     QMU.y=uniq.QMU$QmuCode.y,
                     Method.y = uniq.QMU$Method.y,
                     Quantity.y = uniq.QMU$Quantity.y,
                     Pairs=rep(NA, length(uniq.QMU$QmuCode.x)),
                     slope=rep(NA, length(uniq.QMU$QmuCode.x)),
                     int= rep(NA, length(uniq.QMU$QmuCode.x)),
                     R2=rep(NA, length(uniq.QMU$QmuCode.x)))
#overall correlation
lm1 <- summary(lm(data=Grav.LS, Grav.Avg~LS.Avg))

for (i in 1:length(uniq.QMU$QmuCode.x)){
  df <- Grav.LS[Grav.LS$QmuCode.x==uniq.QMU$QmuCode.x[i] & Grav.LS$QmuCode.y==uniq.QMU$QmuCode.y[i],]
  lm1 <- summary(lm(data=df, Grav.Avg~LS.Avg))
  
  table4e$Pairs[i] <- length(df$ObservedDate)
  table4e$slope[i] <- round(lm1$coefficients[2,1],2)
  table4e$int[i] <- round(lm1$coefficients[1,1],2)
  table4e$R2[i] <- round(lm1$r.squared,3)
}
...
compare diff LS values if there is colocation? 
```{r}
